import base64, binascii, http.server, json, os, secrets, time, urllib.request, urllib.error, uuid
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        streaming=False;job=None;base=None
        try:
            ...
            if route['type']=='openai':
                with request(route['base_url']+'/chat/completions',body,timeout=340) as upstream:
                    self.send_response(upstream.status);self.send_header('Content-Type',upstream.headers.get('Content-Type','application/json'));self.send_header('Connection','close');self.end_headers();self.close_connection=True
                    while True:
            ...
            base=route['base_url'];submitted=rpc(base+'/run',{'input':payload,'policy':{'executionTimeout':600000,'ttl':3600000}});job=submitted['id']
            ...
            while time.monotonic()<deadline:
                ...
                if result['status'] in ['FAILED','CANCELLED','TIMED_OUT']:raise RuntimeError(str(result.get('error',result['status'])))
