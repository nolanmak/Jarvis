...
week) and passes it in explicitly. Standalone/backlog use passes it by hand.
  ...
  send_invoice.py --number 35 --start 2026-05-17 --end 2026-05-24 \
      --to operator-alt@example.com --from-entity <composio_entity>
...
Env:
  ...
  ORCHID_GH_REPO     owner/name slug for `gh` (default OrchidStudio/orchid; no clone needed)
"""
...
def main():
    ...
    subject = f"Invoice #{n} — {fmt(start_d)}-{fmt(end_d)} (Orchid / Glass Entertainment Group)"
