...
| `INVOICE_FROM_ENTITY` | Composio entity/user_id for the sending Gmail (REDACTED) |
| `ORCHID_GH_REPO` | owner/name slug for `gh` (default `REDACTED`; **no local clone needed**) |
| `INVOICE_OUT_DIR` | PDF output dir (default `/tmp/invoices`) |

> Portable by design: PR data is read via `gh pr list --repo REDACTED`,
...
python3 send_invoice.py --number 35 --start 2026-05-17 --end 2026-05-24 \
    --to REDACTED --from-entity <entity>

# real send
python3 send_invoice.py --number 35 --start 2026-05-17 --end 2026-05-24 \
    --to REDACTED --from-entity <entity> --dry-run false
```
