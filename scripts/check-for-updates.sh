...
# daemon so code updates take effect. Intended to be driven by a LaunchAgent
# every 5 minutes (see install-autoupdate.sh) but also runnable manually.
#
# Design notes:
...
# - All output goes to ~/Library/Logs/augmentagent/update.log for post-mortem.
