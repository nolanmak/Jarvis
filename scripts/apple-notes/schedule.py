#!/usr/bin/env python3
"""Install/uninstall the optional macOS Apple Notes export schedule (launchd)."""
import argparse
import os
import plistlib
import subprocess
...
LABEL = "org.augmentagent.apple-notes-sync"


def make_plist(sync_args, interval, home=None):
    home = Path(home) if home is not None else Path.home()
    log = home / "Library/Logs/augmentagent/apple-notes-sync.log"
    return {
        "Label": LABEL,
...
def main():
    ap = argparse.ArgumentParser(description=__doc__, epilog="Pass exporter options after --. See docs/APPLE-NOTES.md.")
    ap.add_argument("--uninstall", action="store_true")
    ap.add_argument("--interval", type=int, default=2, metavar="MINUTES")
    ap.add_argument("sync_args", nargs=argparse.REMAINDER)
    args = ap.parse_args()
    if sys.platform != "darwin":
        ap.error("install this job on the Mac that has Notes")
    if args.interval < 1:
    ...
    if not args.uninstall:
        ...
        sync_args = [a for a in sync_args if a not in ("--db", "--out", str(parsed.db), str(parsed.out))]
    ...
    if args.uninstall:
        ...
        print("schedule removed; exported notes retained")
        return
    os.umask(0o077)
    ...
    print(f"Logs: {log_dir / 'apple-notes-sync.log'}")
    print(f"Scheduled Python executable (for Full Disk Access, if macOS asks): {sys.executable}")
