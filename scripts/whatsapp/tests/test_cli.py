...
import schedule
import sync as cli
from whatsapp_sync import slugify
from test_sync import make_fixture_db, add_message, cd


class CliTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix="whatsapp test ")
        self.addCleanup(self.temp.cleanup)
        self.root = Path(self.temp.name)
        self.db = self.root / "ChatStorage ?#.sqlite"
        con = make_fixture_db(self.db)
        add_message(con, 1, 1, "Synthetic message", cd(1782475200), 0)
        con.close()
    ...
    def test_plist_roundtrip_preserves_paths_and_arguments(self):
        args = ["--out", "/private/path with spaces & chars"]
