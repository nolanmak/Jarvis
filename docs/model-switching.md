...
| --- | --- | --- | --- |
| Selection, restart persistence, pause and snapshot | `model_selection.rs`, `fallback.rs` | `model_selection::tests`, `paused_persisted_runpod_selection_never_reaches_inference_or_fallback`; Linux `discord_model_switch_sequence_uses_one_audited_harness` now overlaps an active GLM turn with `/model set codex` and checks the running turn's GLM usage/tool attribution before the next Codex turn | Live Discord set/call race on the daemon host |
