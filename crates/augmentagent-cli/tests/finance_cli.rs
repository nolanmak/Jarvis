...
fn cli(root: &std::path::Path, args: &[&str]) -> std::process::Output {
    Command::new(env!("CARGO_BIN_EXE_augmentagent"))
        .current_dir(root)
        .env_remove("PLAID_CLIENT_ID")
        .env_remove("PLAID_SECRET")
...
async fn real_cli_queries_and_exports_imported_records_without_bank_secrets() {
    ...
    let mut store = FinanceStore::open(&root.path().join("data.db")).unwrap();
    store
        .add_item("sandbox", "fixture-item", "Household")
        .unwrap();
    ...
    assert_eq!(result["totals"][0]["net_outflow"], "0.3");
    let out = cli(root.path(), &["--wiki-dir", "wiki", "finance", "export"]);
    assert!(
        out.status.success(),
        "{}",
        String::from_utf8_lossy(&out.stderr)
    ...
    assert!(std::fs::read_to_string(wiki.join("index.md"))
        .unwrap()
        .contains("finance/plaid-sandbox-2026-09.md"));
    let out = cli(root.path(), &["--wiki-dir", "wiki", "wiki", "index"]);
    assert!(
        out.status.success(),
        "{}",
        String::from_utf8_lossy(&out.stderr)
    );
    assert!(String::from_utf8_lossy(&out.stdout).contains("finance: 2 pages on disk, 0 missing"));
    let out = cli(root.path(), &["finance", "transactions", "--start", "bad"]);
    assert!(!out.status.success());
    let out = cli(root.path(), &["finance", "check"]);
    assert!(!out.status.success());
    assert!(String::from_utf8_lossy(&out.stderr).contains("PLAID_CLIENT_ID missing"));
}
#[test]
fn cli_rejects_overlapping_sync() {
    let root = tempfile::tempdir().unwrap();
    let lock = std::fs::File::create(root.path().join("data.finance.lock")).unwrap();
    lock.lock().unwrap();
    let out = cli(root.path(), &["finance", "sync"]);
    assert!(!out.status.success());
    assert!(String::from_utf8_lossy(&out.stderr).contains("another finance operation"));
}
