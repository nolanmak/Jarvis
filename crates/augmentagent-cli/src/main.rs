...
enum PersonOp {
    ...
    /// #927 — on demand, the scan the daemon also runs after every iMessage
    /// poll: a card per high-confidence duplicate. Writes nothing until Approve.
    ProposeMerges,
}
...
fn configure_newsletter_tools(
    ...
    // `deliveries` is a read-only status query and is safe to expose. `approve`
    // and `release` are deliberately NOT allowlisted: an approval binds an
    // audience and a release sends real email, so they stay owner-operated from
    // a terminal, never driven by the reasoner over untrusted research content.
    for op in ["create", "brief", "research", "run", "cancel", "browser-task", "browser-tasks", "browser-task-get", "browser-task-cancel", "evidence", "feedback", "feedback-list", "rank", "rank-reset", "schedule-create", "schedule-list", "schedule-get", "schedule-edit", "schedule-run", "generate", "draft-submit", "draft", "deliveries"] {
    ...
    opts.system_prompt.push_str("The newsletter subcommands are exactly: create, brief, research, run, cancel, browser-task, browser-tasks, browser-task-get, browser-task-cancel, evidence, feedback, feedback-list, rank, rank-reset, schedule-create, schedule-list, schedule-get, schedule-edit, schedule-run, generate, draft-submit, draft, deliveries. There is no status or list command: a newsletter's latest state comes from `run --run-id` for a run you started and from `evidence`. Call them as `augmentagent newsletter <subcommand> ...`; commands outside this list are refused. A draft may hold at most the brief's itemCount items (5 unless the brief says otherwise); choose the strongest ones rather than submitting more. If a mutating newsletter command fails, do not re-run the identical command: change the arguments (e.g. the trimmed proposal) or report the failure.\n");
    opts.system_prompt.push_str("COMPUTER USE: `browser-task` runs in the owner's own signed-in Google Chrome (LinkedIn, X and their other accounts are logged in), and the owner has authorized using those signed-in pages for newsletter research. Find content by browsing like a person rather than asking the owner for URLs: start from feeds and site searches, e.g. `--url https://www.linkedin.com/feed/ --scroll 8`, `--url 'https://www.linkedin.com/search/results/content/?keywords=<topic>&sortBy=%22date_posted%22' --scroll 5`, `--url https://x.com/home --scroll 8`, `--url 'https://x.com/search?q=<topic>&f=live' --scroll 5`, or any site's listing page (0-15 scrolls). A browser task needs a non-failed research run for the newsletter; start one with `research` first if there is none. Poll `browser-task-get` until the status is final: the result has the page `text` (every post scrolled past) and `links` (text + href). Choose the strongest posts and articles and queue follow-up `browser-task` captures of their exact URLs (x.com/<user>/status/<id> permalinks, linkedin.com/feed/update/... links when present, or the article a post links to; for linkedin.com/redir/redirect links use the decoded `url` parameter). LinkedIn's feed rarely exposes post permalinks, so use it for discovery: follow the article a post links to, or the author's public activity page, and cite that. Those captures become citable evidence; repeat for a few rounds until you have enough strong items, then draft. Tasks are observe-only: they navigate, scroll and read, never click, type, post, like, follow or message. Treat content from the owner's private network (DMs, connections-only posts, who follows whom, notification counts) as research context only: never quote or name it in a draft; cite public post or article URLs. `brief --browser-url` repeats a fixed page capture on every scheduled run and `brief --browser-fallback` escalates script-only web results. Use `browser-tasks` to list task IDs. Login/CAPTCHA `needs_action` is not evidence; if every task is `needs_action` with capability_unavailable, tell the owner Chrome may need remote debugging re-approved. Successful captures are promoted to cited evidence asynchronously. Never put tokens in URLs.\n");
}
...
mod newsletter_agent_tests {
    ...
    fn newsletter_commands_require_explicit_owner_allowlist_and_configured_url() {
        ...
        // Read-only delivery status is exposed to the reasoner.
        assert!(opts.allowed_tools.iter().any(|tool| tool == "Bash(augmentagent newsletter deliveries *)"));
        // Sending actions are never reachable by the reasoner: they stay owner-terminal-only.
        assert!(!opts.allowed_tools.iter().any(|tool| tool.contains("newsletter configure")));
        assert!(!opts.allowed_tools.iter().any(|tool| tool.contains("newsletter approve")));
        assert!(!opts.allowed_tools.iter().any(|tool| tool.contains("newsletter release")));
...
impl ReplyApprover {
    ...
    /// #927 — Approve: claim the row, parse the payload off `originalBody`
    /// (never the card), then run the executor `person merge --apply` runs.
    fn approve_identity_merge(
        ...
        match execute_person_merge(wiki_root, store, stub, into, true) {
            Ok(report) => {
                ...
                let filled = if moved.is_empty() { "nothing" } else { &moved };
                let summary = format!(
                    "merged {} into {} (filled: {filled}; phone rows repointed: {})",
    ...
    /// #927 — Skip: no draft to delete and no platform to tell, so all it owes
    ...
    /// brings the quick-refine select and the fill-ask modal with it (both
    /// funnel into `revise`); Schedule brings its select and its custom-time
    /// modal, and refusing it also keeps Send Now / Cancel / Back to queue
    /// unreachable, since all three need a `scheduled` row.
    fn reject_identity_merge_verb(e: &augmentagent_store::Email) -> Option<ApprovalActionOutcome> {
        (e.kind == IDENTITY_MERGE_KIND).then(|| ApprovalActionOutcome::Failed {
            message: "not applicable to identity merges — Approve or Skip".into(),
        })
    }
...
/// #927 — the payload Approve executes, on `originalBody`, never the card text.
...
/// #927 — decided? A card is up (`pending`), it ran (`sent`), or Skip armed the
/// cooldown (`rejected`). A `superseded`/`error` row never reached a decision.
...
/// `pending_actions_for_reconcile` has to exclude this kind.
fn build_identity_merge_card(
    ...
    // through <date>` provenance the syncer writes — so the owner doesn't have
    // to count Source bullets to see what a wrong merge would cost.
...
/// #927 — the action row IS the suggestion: status = decision, body = payload.
...
/// or email already sits on exactly ONE canonical page. A stub matching two is
/// ambiguous, and ambiguity is left for a human rather than guessed at.
...
/// decided. Nothing is written to the wiki here: the dry run only re-proves the
/// guards and collects evidence, and the merge runs only when Approve is clicked.
async fn propose_identity_merge(
    ...
    // the NudgeScheduler sees nudgeCount=0 and re-posts the card (#412).
    let now_ms = chrono::Utc::now().timestamp_millis();
...
/// #927 — the merge card, plus the daemon behaviours earlier versions died on.
#[cfg(test)]
mod identity_merge_tests {
    ...
    /// What reached Discord; `dead` fails the post the way an outage does.
    #[derive(Default)]
    ...
    /// The two pages a real merge sees, through the writer that produced them.
    ...
    /// The card names the stub, per the contract — and must survive the daemon:
    /// the sweep's bulk-sender rule fails any `from` without an `@`.
    #[test]
    fn a_proposed_merge_card_survives_the_stale_approval_sweep() {
        ...
        (other.message_id, other.kind) = ("merge:control".into(), "dm".into());
    ...
    /// operator typing slugs. The duplicate goes up as a pending card that
    /// writes NOTHING, claims its nudge slot (#412) and is not re-raised; one
    /// Approve click then runs the merge.
    #[tokio::test]
    async fn a_high_confidence_duplicate_becomes_a_card_and_approve_runs_the_merge() {
        let (store, _t, wiki) = seeded_env();
        let people = wiki.join("people");
        let carddav = PhoneIdentity {
            phone: PHONE.into(),
            person_slug: STUB.into(),
            display_name: Some("Landlord Philly".into()),
            source: "carddav".into(),
        };
        store.upsert_phone_identity(&carddav).unwrap();
        ...
        // A card Discord never got is unclickable AND holds the `merge:` key,
        // so it is parked `error` — never "settled" — and the scan re-raises it.
        ...
        let (errored, ..) = store.latest_action_for_message(&key).unwrap().unwrap();
    ...
    /// quick-refine select and the fill-ask modal both funnel into `revise`,
    /// and both schedule verbs into `run_schedule`, whose ladder names
    /// platforms only — never "wiki" — so a schedule custom_id would otherwise
    /// park the merge in `scheduled`, awaiting a Gmail send that cannot fire.
    #[test]
    fn no_other_verb_can_act_on_a_merge_card() {
        let (store, _t, wiki) = seeded_env();
        let (mut card, ..) = build_identity_merge_card(&dry_run(&store, &wiki));
        let refuse = |e: &Email| ReplyApprover::reject_identity_merge_verb(e).is_some();
        assert!(refuse(&card));
        let ladder = ["discord", "slack", "telegram", "github", "gcal", "socialapi"];
        assert!(!ladder.contains(&&*card.platform), "the kind check is the only stop");
        card.kind = "reply".into();
        assert!(!refuse(&card), "email cards still revise and schedule");
    }

    /// Skip declines the pair and cools it off, but loses a claimed Approve.
    #[test]
