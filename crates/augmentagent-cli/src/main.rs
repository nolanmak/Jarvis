...
#[cfg(test)]
mod compose_subject_tests {
    //! #675: a new-email compose card (#412) keys its action on a synthetic
    //! `compose:` message id whose `emails.subject` is the OUTGOING subject.
    //! Revise recreated the draft as `Re: <subject>` — right for a reply card,
    //! but on a fresh compose it turned "Ship Systems x EFS" into
    //! "Re: Ship Systems x EFS" and Approve sent that.

    use super::{card_original_subject, revise_draft_subject};
    use augmentagent_store::Email;

    fn email(message_id: &str, thread_id: Option<&str>, subject: &str) -> Email {
        Email {
            message_id: message_id.into(),
            thread_id: thread_id.map(str::to_string),
            from: "evan@efsnetworks.com".into(),
            to: String::new(),
            cc: String::new(),
            subject: subject.into(),
            body: String::new(),
            date: String::new(),
            account_entity_id: Some("ent-1".into()),
            platform: "gmail".into(),
            kind: "dm".into(),
        }
    }
    ...
    #[test]
    fn revise_subject_keeps_new_email_compose_subject_verbatim() {
        assert_eq!(
            revise_draft_subject(&email("compose:draft-123", None, "Ship Systems x EFS")),
            "Ship Systems x EFS"
        );
    }

    #[test]
    fn revise_subject_prefixes_a_reply_to_a_real_message() {
        assert_eq!(
            revise_draft_subject(&email("18f2a1b3c4d5e6f7", Some("18f2a1b3"), "Ship Systems x EFS")),
            "Re: Ship Systems x EFS"
        );
    }

    #[test]
    fn revise_subject_never_double_prefixes_a_reply() {
        assert_eq!(
            revise_draft_subject(&email("18f2a1b3", Some("18f2a1b3"), "Re: hi")),
            "Re: hi"
        );
        assert_eq!(
            revise_draft_subject(&email("18f2a1b3", Some("18f2a1b3"), "RE: hi")),
    ...
    #[test]
    fn revise_subject_keeps_a_deliberate_re_prefix_on_a_new_email() {
        assert_eq!(
            revise_draft_subject(&email("compose:draft-9", None, "Re: Ship Systems x EFS")),
            "Re: Ship Systems x EFS"
        );
    }

    #[test]
    fn new_email_card_stores_the_outgoing_subject_verbatim() {
        assert_eq!(
            card_original_subject("Ship Systems x EFS", None, false),
            "Ship Systems x EFS"
        );
        assert_eq!(
            card_original_subject("Re: Ship Systems x EFS", None, false),
            "Re: Ship Systems x EFS"
        );
    }

    #[test]
    fn reply_card_still_stores_the_original_inbound_subject() {
        assert_eq!(
            card_original_subject("Re: Ship Systems x EFS", None, true),
            "Ship Systems x EFS"
        );
        assert_eq!(
            card_original_subject("re: Ship Systems x EFS", None, true),
        ...
        assert_eq!(
            card_original_subject("Re: anything", Some("Original ask"), true),
            "Original ask"
        );
        // Subject that is exactly "Re:" — stripping leaves nothing to store.
        assert_eq!(card_original_subject("Re:", None, true), "Re:");
    }
}
