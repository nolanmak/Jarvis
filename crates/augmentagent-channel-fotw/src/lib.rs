...
mod tests {
    ...
    /// `fotw:<id>`, but the side that *writes* the citation is the ingest
    /// agent, which learns the rule only from `schema/wiki-skill.md` — read at
    /// runtime, so dropping it costs every meeting fact its attribution
