...
use augmentagent_channel_core::{HaltReason, Platform, RateGovernor, Reasoner, ReasonerOpts};
...
pub struct InstagramChannel<A: InstagramApi, R: Reasoner> {
    ...
    /// gh CLI runner used by `handle_code_mode_failure` / `report_classic_fallback`
    /// to file postmortem issues when code-mode fails. Default is
    /// [`GhCliIssueRunner`] which shells out to the `gh` binary on PATH; tests
    /// swap in a recorder via [`InstagramChannel::with_gh_issue_runner`].
...
impl<A: InstagramApi, R: Reasoner + 'static> InstagramChannel<A, R> {
    ...
    /// Override the gh CLI runner — production keeps the default
    /// [`GhCliIssueRunner`], tests inject a recorder to assert postmortems
    ...
    async fn handle_dm(&self, dm: Dm) -> anyhow::Result<Option<Dispatch>> {
        ...
        match decision.decision {
            ...
            DecisionKind::Reply => {
                ...
                // --- CODE-MODE attempt (I11). Two-step flow:
                //   1. Reasoner emits a TypeScript program that orchestrates
                //      tool calls and ends with `tools.draft("instagram", body, reason)`.
                ...
                // `handle_code_mode_failure` (I7) for one self-repair pass.
                // If the repair lands a working code-mode draft, we use it.
                // Otherwise we fall through to the classic draft path AND, after
                // the classic row lands, call `report_classic_fallback` to file
                // the postmortem gh issue + Discord notice.
                //
                // Instagram channel-specific notes:
                // - All context blocks (wiki/tone/thread/archetype/resolved)
                //   are empty, mirroring the channel's classic call which
                //   already passes "" for each. The blocks render to no output
                //   so the cache prefix is stable.
                // - The live-send arming gate (`INSTAGRAM_REAL_ACCOUNT_ENABLED`)
                //   lives in `composer`, on the dispatch entry, not here —
                //   code-mode does not double-gate.
                let manifest = manifest_v1();
                let system_prompt = code_mode_system(&manifest);
                let user_msg = code_mode_user_message(&email, "", "", "", "", "");
                // Opts mirror the classic `draft_opts` shape: same default
                ...
                let (code_mode_action_id, pending_classic_record): (
                    ...
                    Err((cme, stage)) => {
                        warn!(
                            ...
                            stage = ?stage,
                            "instagram code-mode attempt failed: {cme}; invoking self-repair"
                        );
                            ...
                            .await
                        {
                            DraftOutcome::CodeMode {
                                ...
                                info!(
                                    ...
                                    action_id = %action_id,
                                    repair_used,
                                    "instagram code-mode self-repair succeeded"
                                );
                                (Some(action_id), None)
                            }
                            DraftOutcome::ClassicNeeded(record) => {
                                error!(
                                    message_id = %email.message_id,
                                    "instagram code-mode self-repair failed; falling back to classic"
                                );
                                (None, Some(record))
                            }
                        }
                ...
                // out of the actions row the dispatcher just wrote, then hand
                // off to the existing approval flow.
                if let Some(action_id) = code_mode_action_id {
                    ...
                    // Non-dry-run: dispatcher already wrote a Pending row, so
                    // we only need to post the approval card + record the nudge.
                    if let Err(e) = self
                        .approvals
                    ...
                    info!(
                        action_id,
                        message_id = %email.message_id,
                        "instagram approval card posted (code-mode)"
                    );
                    return Ok(Some(Dispatch::AwaitingApproval));
                }

                // --- Classic fallback path. Reached when code-mode failed AND
                // self-repair didn't produce a working code-mode draft.
                // Behaviour matches the pre-I11 classic prompt path; after the
                // action row lands we call `report_classic_fallback` to file
                // the postmortem gh issue + post the Discord notice tagged
                // with the classic action_id.
                let draft_opts =
