...
use crate::sigextract::{is_cc_only_bystander, is_event_blast, is_human_sender, is_meeting_invite};
    ...
    pub async fn process_email(
        ...
        match decision.decision {
            ...
            DecisionKind::Reply => {
                ...
                // Scoped to the account the message arrived on: in a
                // multi-account deployment, mail delivered to A but addressed
                // to B (also active) must not read as "A is on To". No
                // account id on the row (legacy) ⇒ every active account.
                    ...
                    Ok(accounts) => accounts
                        ...
                        .filter(|a| {
                            email
                                .account_entity_id
                                .as_deref()
                                .is_none_or(|own| own == a.entity_id)
                        })
                        .map(|a| a.email)
                        .collect(),
                    Err(e) => {
                ...
                let wiki_hint = self
                    ...
                    .map(|root| {
                        let layout = augmentagent_wiki::WikiLayout::new(root.clone());
                        augmentagent_wiki::WikiReader::new(&layout)
                            .with_transcripts_dir(
                                augmentagent_wiki::transcripts_meetings_dir_from_env(),
                            )
                            .draft_hint(&email)
                    })
                    .unwrap_or_default();
