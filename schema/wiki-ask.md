...
- **Create:** `augmentagent loop create --interval <secs|30m|2h|1d> --prompt "<text>"` — writes a new active row to `user_loops`. Owner + channel default to the env-configured Discord identity, so for normal "schedule a thing for me" asks you don't need `--owner` / `--channel-ref`. Add `--expires-in <duration>` for "stop it after a week" / "for the next 3 days" asks (accepts the same grammar as `--interval`). The command prints the new loop UUID on stdout — surface it back to the user.
...
- **Read back before you create.** Mirror `set-recipient`: before calling `loop create`, confirm the interval and prompt with the user in a one-line read-back ("creating: every 24h, prompt `morning`. OK?"). The cadence and the prompt are the two things that actually matter — if either is wrong the loop spams Discord on the wrong schedule with the wrong content. Don't surface the optional flags (`--owner`, `--channel-ref`) in the read-back; they default to the user's identity and are noise.
- **`/loop` is interval-based, not cron.** If the user asks for "every Monday morning" or "every weekday at 9am", don't try to fudge it with an interval. Tell them `/loop` only supports fixed cadences (e.g. `1d`, `12h`, `30m`) and ask whether they want the closest interval approximation (e.g. "every 24h, starting now") or whether they'd rather wait for cron-style scheduling.
